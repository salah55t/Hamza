        ...
        if isinstance(msg, list):
            for m in msg:
                ...
                if symbol:
                    ...
                    logger.debug(f"تم تحديث السعر لـ {symbol}: {last_price_update[symbol]}")
        else:
            symbol = msg.get('s')
            if symbol:
...
def calculate_ema(series, span):
    ...
    logger.debug(f"EMA({span}) = {ema.iloc[-1]:.4f}")
    return ema
...
def calculate_rsi_indicator(df, period=7):
    ...
    rsi = 100 - (100 / (1 + rs))
    logger.debug(f"RSI({period}) = {rsi.iloc[-1]:.2f}")
    return rsi
...
def calculate_atr_indicator(df, period=7):
    ...
    logger.debug(f"ATR({period}) = {df['atr'].iloc[-1]:.8f}")
    return df
...
def calculate_macd_indicator(df, fast=12, slow=26, signal=9):
    ...
    logger.debug(f"MACD = {macd.iloc[-1]:.4f}، Signal = {signal_line.iloc[-1]:.4f}")
    return df
...
def calculate_stochastic(df, k_period=14, d_period=3):
    ...
    logger.debug(f"Stochastic %K = {df['%K'].iloc[-1]:.2f}، %D = {df['%D'].iloc[-1]:.2f}")
    return df
...
def calculate_confidence_score(indicators, candle_pattern, risk_reward_ratio, volatility):
    ...
    logger.debug(f"درجة الثقة: {final_score}")
    return final_score
...
def fetch_historical_data(symbol, interval='5m', days=3):
    ...
    if cache_key in historical_data_cache:
        logger.info(f"{symbol}: تم جلب البيانات التاريخية من الكاش")
...
def analyze_market():
    ...
    try:
        ...
        for symbol in symbols:
            ...
            if df is None:
                logger.info(f"{symbol}: رفض التوصية - عدم توفر البيانات التاريخية")
                continue
            if len(df) < 100:
                logger.info(f"{symbol}: رفض التوصية - البيانات التاريخية غير كافية (عدد الصفوف: {len(df)})")
                continue
