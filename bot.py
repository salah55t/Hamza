...
# ثابت لحجم الصفقة الافتراضي (يمكن تعديله أو تحميله من الإعدادات)
TRADE_VALUE = 100.0
...
# دالة للتأكد من اتصال قاعدة البيانات
...
def set_telegram_webhook():
    webhook_url = "https://hamza-o84o.onrender.com/webhook"
...
def send_telegram_alert(signal, volume, btc_dominance, eth_dominance, timeframe):
    try:
        equity = 10000  # قيمة الرصيد الإجمالي (يمكن استبدالها بقيمة فعلية)
...
——————————————
**زوج:** {signal['symbol']}
**سعر الدخول:** `${signal['price']:.8f}`
**🎯 الهدف:** `${signal['target']:.8f}` (+{profit_pct*100:.2f}% / +{profit_usdt} USDT)
**🛑 وقف الخسارة:** `${signal['stop_loss']:.8f}` ({loss_pct*100:.2f}% / {loss_usdt} USDT)
**⏱ الفريم:** {timeframe}
**💧 السيولة:** {volume:,.2f} USDT
**💵 حجم الصفقة:** ${trade_size:.2f}
——————————————
📈 **نسب السيطرة (15m):**
   • BTC: {btc_dominance:.2f}%
   • ETH: {eth_dominance:.2f}%
📊 **مؤشر الخوف والجشع:** {get_fear_greed_index():.2f}
——————————————
⏰ **{timestamp}**"""
        reply_markup = {
            "inline_keyboard": [
...
# ---------------------- دوال إضافية لتفادي أخطاء وقت التشغيل ----------------------
def run_flask():
    app.run(host='0.0.0.0', port=5000)

def test_telegram():
    logger.info("✅ اختبار Telegram تم بنجاح.")

def analyze_market():
    logger.info("🔍 تحليل السوق جارٍ...")
